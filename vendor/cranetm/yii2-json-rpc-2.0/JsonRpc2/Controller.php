...
use app\api\components\Validate;
...
class Controller extends \yii\web\Controller
{
    ...
    private function getActionResponse($requestObject)
    {
        ...
        try {
            ...
            $params=Utils::objectToArray($this->requestObject);
            $method    = $params['method'];
            $timestamp = $params['timestamp'];
            if( !Validate::checkSign($params) )
            {

                $error = new Exception("签名错误", Exception::INTERNAL_ERROR);
            }

            if( !is_numeric($timestamp) )
            {
                $error = new Exception("时间格式错误（包含非数字的字符", Exception::INTERNAL_ERROR);
            }

            if( time() - intval($timestamp) > 300 )
            {
                $error = new Exception("请求已超时", Exception::INTERNAL_ERROR);
            }
            ...
            if($result['status']==false){
                $error = new Exception($result['message'], Exception::INTERNAL_ERROR);
            }

            $check=$this->action->check($this->requestObject->params);
            ...
            foreach($check as $key=>$val){
                ...
                if(false===$val['status']){
                    ...
                    array_push($checkError,$val['message']);
                }

            }

            if(count($checkError)>0){
                $error=new Exception(json_encode($checkError), Exception::INTERNAL_ERROR);
            }
...
//            return null;
    ...
    public function bindActionParams($action, $params)
    {
        try {
            ...
            foreach ($method->getParameters() as $param) {
                $name = $param->getName();
                $args[] = $actionParams[$name] = $this->requestObject->params;
    ...
    private function parseAndValidateRequestObject($requestObject)
    {
        ...
        if (!is_object($requestObject)
            || !isset($requestObject->timestamp)||!isset($requestObject->sign_type)||!isset($requestObject->sign)
